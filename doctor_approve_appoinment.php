<?php
	...
	if( $_SERVER['REQUEST_METHOD'] === "POST" ){
			...
			$stmt = $conn->prepare(" UPDATE appointments SET status = 'APPROVE' WHERE id = ?");
			$stmt->execute( [ $_POST['id'] ] );
			// counting the affected rows
			if( $stmt->rowCount() > 0 ){
				$_SESSION['success'] = 1;
				header("Location: doctor_approve_appoinment.php");
				die();
			}
			else{
				$_SESSION['success'] = 0;
	...
	<div class="welcome-text">
		...
		<div style="margin-left: 5%; margin-right: 5%;">
				<form action="" method="post">
					...
					<input type="submit" value="Approve Appoinment" name="approve_appoinment" style="background-color: #44c38d;">
				</form>	
		</div>
